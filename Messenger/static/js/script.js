...
imageInput.addEventListener('change', (event) => {
  ...
  if (file) {
    const fileName = file.name.toLowerCase();  // Ensure case-insensitive check
    const fileType = file.type;

    // Debug: Log the file name to verify it's correct
    console.log('Selected file:', fileName);
    console.log('Select file type;', fileType)
    // Check for image extensions
    if (fileName.endsWith('.jpg') || fileName.endsWith('.png')) {
      formData.append('img', file);     
      console.log('File is an image:', fileName);  // Debug: log for image type
    }
    // Check for video (.mp4) extension
    else if (fileName.endsWith('.mp4')||fileType==='video/mp4') {
      formData.append('video', file);
      console.log('File is a video:', fileName);  // Debug: log for video type
    }
    // If the file is neither an image nor an mp4 video, log an error
    else {
      console.error('Error: Unsupported file type. Only .jpg, .png, .mp4 are allowed.');
      return;  // Exit early if the file type is unsupported
    }

   // If valid file is selected, send the data to the server
