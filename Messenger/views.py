...
def image_send(request,pk):
    ...
    # Check for the files in the request
    ...
    if img_data:
        ...
        video = default_storage.save('file/video/'+video_data.name, video_data)
        Private_messages.objects.create(room=room, video=video, user=request.user)
    else:
        print('error')
